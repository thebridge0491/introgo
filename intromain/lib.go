...
func (tup Tuple2) String() string {
	return fmt.Sprintf("(%s, %s)", tup.t1, tup.t2) }

// MapToSlice ...
func MapToSlice(mapX map[string]interface{}) []interface{} {
	var sliceX = make([]interface{}, len(mapX))
	for k, v := range mapX {
		tup := Tuple2{k, v}
...
func SortableMapToSlice(isSorted bool, mapX *SortableMap) []interface{} {
	var sliceX = make([]interface{}, len(mapX.Keys))
	if isSorted {
...
func ListToSlice(xs *list.List) []interface{} {
	var sliceX = make([]interface{}, xs.Len())
	for el := xs.Front(); nil != el; el = el.Next() {
		sliceX = append(sliceX, el)
	}
	return sliceX
}
...
func RingToSlice(ringX *ring.Ring) []interface{} {
	var sliceX = make([]interface{}, ringX.Len())
...
func HeapToSlice(heapX *Heap) []interface{} {
	var sliceX = make([]interface{}, len(*heapX))
